...
def get_stock_details(tickers):
    ...
    for t in tickers:
        try:
            # 取得をより確実にするためTickerオブジェクトを生成
            tk = yf.Ticker(t)
            info = tk.info
            # 必要な情報が取れない場合でも最低限の枠を作成
            details.append({
                "銘柄": t,
...
with st.sidebar:
    ...
    if selected_tickers:
        # spinnerで取得中であることを明示
        with st.spinner("銘柄データを読み込み中..."):
            stock_df = get_stock_details(selected_tickers)
        
        # 取得したデータに基づいて入力欄を強制的に作成
        for t in selected_tickers:
            # stock_dfから該当銘柄の情報を探す
            row = stock_df[stock_df['銘柄'] == t].iloc[0] if not stock_df.empty and t in stock_df['銘柄'].values else {"名称": t, "銘柄": t, "セクター": "不明", "現在値": 0}
            
            with st.expander(f"{row['名称']} ({t})", expanded=True):
                qty = st.number_input("保有数", min_value=0, step=10, key=f"sidebar_q_{t}")
                price = st.number_input("取得単価", min_value=0.0, step=1.0, key=f"sidebar_p_{t}")
                portfolio_inputs[t] = {"qty": qty, "price": price, "current": row['現在値'], "name": row['名称'], "sector": row['セクター']}
    else:
...
with tab2:
    ...
    if not portfolio_inputs:
        ...
        if total_val > 0:
            ...
            st.subheader("保有銘柄一覧")
            st.dataframe(p_df.style.format({"現在値": "{:,.1f}", "評価額": "{:,.0f}", "損益": "{:,.0f}"}))
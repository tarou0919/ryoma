...
tab1, tab2 = st.tabs(["📈 株価分析", "💰 ポートフォリオ管理"])
...
with tab2:
    ...
    if not selected_tickers:
        ...
        if not stock_df.empty:
            st.subheader("保有状況を入力してください")
            
            portfolio_data = []
            # 1銘柄ごとに1行の入力欄を作成
            for _, row in stock_df.iterrows():
                col_name, col_qty, col_price = st.columns([2, 1, 1])
                with col_name:
                    st.write(f"**{row['名称']}** ({row['銘柄']})")
                    st.caption(f"現在値: {row['現在値']:,}円 / {row['セクター']}")
                with col_qty:
                    qty = st.number_input("保有数", min_value=0, step=10, key=f"q_{row['銘柄']}")
                with col_price:
                    price = st.number_input("取得単価", min_value=0.0, step=1.0, key=f"p_{row['銘柄']}")
                
                eval_val = qty * row['現在値']
                profit = (row['現在値'] - price) * qty if price > 0 else 0
                portfolio_data.append({**row, "保有数": qty, "取得単価": price, "評価額": eval_val, "損益": profit})
            ...
            if total_val > 0:
                ...
                st.info("保有数を入力すると、資産配分のグラフが表示されます。")